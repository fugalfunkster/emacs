...
#                       Claude launch; chart file creation and archiving; venue
#                       worktree creation; gathering state; bebop-kill-session
#                       with full cleanup.
#     bebop-cue.el      bebop-cue-mode minor mode; C-c C-p / C-c C-P (cue) and
#                       C-c C-j / C-c C-J (jam); add-hook for org-mode.
#     bebop-dashboard   bebop-repos-dir updated to ~/Code/Repos/;
#                       bebop-dot-pending-face added (yellow/amber);
#                       bebop--infer-status checks bebop gathering state first;
#                       bebop--insert-session-line renders yellow dot for
#                       gathering status.
#
#   Phase 4 — Startup + Frame
#     bebop-frame.el    M-x bebop opens the Conductor frame (locked 3-window
#                       layout: chart left, *bebop* top-right,
#                       *bebop-session: NAME* bottom-right); session switch
#                       updates chart and composition windows; M-x bebop-solo
#                       NAME opens/focuses a per-session Solo frame (treemacs
...
#   - bebop-repos-dir default changed in-place in the Customization block
