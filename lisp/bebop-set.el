...
(defun bebop-set--gutter (row)
  "Return the fixed-column gutter string for ROW, or blanks if ROW is nil."
  (if (null row)
      (make-string 7 ?\s)
    (let* ((live (plist-get row :live))
           (shelved (plist-get row :shelved))
           (dot (cond
                 (live (propertize "●" 'face (bebop-set--dot-face
                                              (plist-get live :status))))
                 (shelved (propertize "○" 'face '(:inherit shadow :slant italic)))
                 (t (propertize "○" 'face 'shadow))))
           (chart (if (plist-get row :chart)
                      (propertize "⊞" 'face 'shadow) " "))
           (venue (if (plist-get row :venue)
                      (propertize "⎇" 'face 'shadow) " ")))
      (concat dot " " chart " " venue "  "))))
  ...
  (let* ((name (plist-get row :name))
    ...
    (magit-insert-section (bebop-session-row name)
      ...
      (when ports
        ...
        (insert (propertize (mapconcat (lambda (p) (format ":%d" p)) ports " ")
                            'face 'shadow)))
      (insert "\n")
      (add-text-properties
       start (point)
    ...
    (magit-insert-section (bebop-set set hide)
      (magit-insert-heading
        (concat (bebop-set--gutter own)
                (propertize set 'face '(:weight bold))
                (make-string (max 2 (- width (length set))) ?\s)
                (propertize (bebop-set--counts rows) 'face 'shadow)))
      (when own
        (add-text-properties
         start (point)
        ...
        (magit-insert-section (bebop-shelved (concat set "/shelved") t)
          (magit-insert-heading
            (concat (make-string 7 ?\s) "    "
                    (propertize (format "… %d shelved" (length shelved))
    ...
    (magit-insert-section (bebop-setlist)
      ...
      (when-let ((un (plist-get groups :ungrouped)))
        (magit-insert-section (bebop-ungrouped "Ungrouped" t)
          (magit-insert-heading
            (concat (make-string 7 ?\s)
                    (propertize "Ungrouped" 'face '(:inherit shadow :weight bold))
                    (make-string (max 2 (- width 9)) ?\s)
                    (propertize (bebop-set--counts un) 'face 'shadow)))
          (dolist (r un)
            (bebop-set--insert-row r width "    "))))
      (when-let ((shelf (plist-get groups :shelf)))
        (magit-insert-section (bebop-shelf "Shelf" t)
          (magit-insert-heading
            (concat (make-string 7 ?\s)
                    (propertize "Shelf" 'face '(:inherit shadow :weight bold))
                    (make-string (max 2 (- width 5)) ?\s)
                    (propertize (format "%d sets" (length shelf)) 'face 'shadow)))
          (dolist (g shelf)
