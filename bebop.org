...
| *Shelved*   | Visibility flag hiding a session or set from the default view; orthogonal to liveness; artifacts untouched       |
...
- *Liveness* — running or not; a fact derived from tmux.
- *Shelved* — hidden from the default view or not; a stored choice.

The old "On deck" section conflated these ("backgrounded, will return" vs.
"abandoned, needs exile"). In the setlist model, objects never move: a
session renders under its set for its entire lifetime, and shelving or
killing it only changes how its line displays. Shelving a set batch-flags
its member sessions — there is no separate set-level flag; a set shows on
the Shelf exactly when it has members and all of them are shelved.

The dashboard is one folding tree (magit-section, TAB to fold):

#+begin_example
  ● ⊞ ⎇  lease-explorer            2 running · 1 shelved
...
gutter; otherwise its gutter is blank.

Set membership and shelved flags persist to =bebop-state.json= (version 2)
— they are choices, not derivable state. Everything else remains derived.
Assignment is designed to be filled asynchronously by agents (silent Jira
epic enrichment, later phase); until then sessions land honestly in
*Ungrouped*, collapsed by default.
...
| =bebop-set=       | =bebop-session=, =magit-section=                  |
...
Sets (attention-level session grouping), shelving (visibility flags), and
the setlist tree renderer. See the =* Sets and the Shelf= design section.
...
2. *Objects never move.* A session renders under its set for life; shelving
   and liveness only change how the line displays, never where it lives.

Only choices persist (set membership, shelved flags, declared sets) — to
...
The mutating verbs (=bebop-assign-set=, =bebop-shelve=, =bebop-unshelve=,
=bebop-new-set=, =bebop-backline-run= …) already live in their owning
modules and are equally part of the contract; this module adds the
read/orientation side plus the exile-proposal queue.
