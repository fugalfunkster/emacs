...
- Sidecar location: move under the synced org tree now (Live Remote
  reads it free) or at phase 4.
...
visited, or unresolved MR comments you haven't seen (stale MR data
never qualifies — old news doesn't earn the lane). A projection, not
a move: the row still renders in its home section; the marquee is a
second listing whose =RET= lands in the same place. Visiting acks,
acking leaves the lane — the amber dot or ◆ stays in the tree, so
nothing is forgotten, only quieted. Ordered worst-first (blocked,
waiting, comments), most-recent within a rank.
