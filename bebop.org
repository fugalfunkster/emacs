...
*The dashboard runs at two speeds.* The left gutter is the fast lane:
local, derived, recomputed every render — what your agents are doing right
now. The right column is the slow lane: MR review status, right-anchored
off the name column, fetched from GitLab out of band by roundup and read
here from a sidecar cache. Its glyphs — red =◆N= (N unresolved teammate
comments; you are needed), green =✓= (merged), dim =✎= (draft), dim =◇=
(open, nothing for you) — reuse the gutter's face vocabulary, so red means
"needs you" on both edges. A cached status older than
=bebop-mr-stale-hours= dims, because the right edge is a snapshot, not a
live signal, and must never pretend otherwise. Backline ports trail the MR
glyph in the same right region.
