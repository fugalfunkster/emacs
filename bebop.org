...
plays at home (the iMac, resident host, always on), and you tune in
from wherever the laptop happens to be.
...
your MacBook Emacs drives the iMac's sessions as if it were sitting at
the iMac, at conversation speed, over a network you don't trust to stay
up.
...
- /Dual-role machines/: the MacBook still runs full local Bebop when
  offline. Is remote a *mode* (one dashboard, host switchable) or a
  *parallel surface* (=*bebop-remote*= alongside =*bebop*=)? Leaning
  parallel surface first — simpler, honest about what's where — with
  merge as a later possibility.
