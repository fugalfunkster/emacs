...
*Choices* are the exception: set declarations, set membership, shelved flags,
and exile proposals persist to =bebop-state.json= (version 2, owned by
=bebop-set=). Never persist anything derivable — only choices.
...
- Comment out =bebop-set= → dashboard reverts to the flat pair list; sets/shelving gone
...
* Tangle artifacts                                                 :noexport:
  ...
  - [[*Bebop — Sets (bebop-set.el)][Bebop Sets]] (=bebop-set.el=) — sets, shelving, setlist tree, persistence
...
4. Press =M-a= (bebop-passthrough) to forward keystrokes for TUI prompts without
   leaving Emacs
5. =TAB= folds sets in the dashboard; =z=/=Z= shelve; the Shelf and Ungrouped
   sections keep in-flight work off stage until it needs you
