...
- Remote Login (sshd) on, Tailscale up. /Status: sshd is on; the Pro
  is *not* on the tailnet yet — today's reach is LAN-only, so the
  cafe story waits on Tailscale enrollment./
...
- /RAM/: 32GB hosts the fleet today; if concurrent Java builds +
