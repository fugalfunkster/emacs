...
One verb joins the allowlist: =relay-done SESSION FROM-HOST= — the
relaying machine's ack after recreating a venue. Its handler only
appends a =retire= proposal to the *receiving* machine's own ledger;
the human rules on it at the next roundup checkpoint. Remote input
stages, it never executes — the same stance as cron roundup.
