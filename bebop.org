...
member, then the set goes with them. A shipped epic is one gesture. The
prompt names any member whose venue holds uncommitted work, and each
teardown is caught: one member that will not go down is reported by
name, and the rest still go. =bebop-set-prune-empty= is the sweep behind
it, exposed to agents as a citizen verb so roundup can prune what
teardown never saw.

*Sticky sets.* The sweep rests on a claim that only holds for one kind of
set: that removal is free because enrichment re-declares the set the
next time a ticket matches its epic. Nothing re-declares a set you typed
in yourself. So a set declared through =bebop-new-set= carries =:sticky t=
in the state file, and both prune paths skip it — an empty hand-built
box stays on stage, waiting for the sessions you built it for.
Enrichment-declared sets are unchanged, and =bebop-remove-set= still
removes anything you point it at. The key is absent on every set written
before the flag existed, and absent reads as not sticky, so nothing
already on disk changes behavior.
...
is playing. It covers two layers that share one truth source:

1. *Venue work shells.* A named tmux window per venue where long-running or
   port-binding processes run — dev servers, watch-mode builds, test
   watchers. Session-scoped: created on first use, killed with the session.
2. *The machine service fleet.* The standing services this machine runs —
   backends, APIs, frontends — which outlive every session. Machine-scoped:
   a session *checks a service out* to take restart authority over it, and
   hands it back on release or teardown.

The problem both layers solve: processes are anonymous. A dev server belongs
to whatever shell happened to start it, so freeing a port means hunting for
the responsible shell — and killing blind is faster, which orphans work. The
backline makes processes *owned*: the registry can always name who holds
:8099 before anything dies.
...
- *The holder is derived, never stored.* There is no lease ledger. A
  service's holder is computed on every read: port → listening pid → if that
  pid sits under a =backline--SLUG= pane, the holder is that venue; else if
  its cwd or command path is under a venue worktree, that venue; else the
  *machine*. A stale lease file cannot lie about a service, because there
  isn't one.
- *Checkout is per service, not per fleet.* Checking out =payment= boots
  whoever held =payment= off =payment= only. Everything else on the machine
  keeps running. Holding a service means restart authority over it, nothing
  more.
- *The machine outlives the session.* Services held by the machine are
  invisible to session teardown — killing, exiling, or archiving a session
  drops only the services that session held, and (by default) asks the
  machine to restore each one to its default state. Nothing gets orphaned
  because nothing session-shaped owned the fleet.
...
lsof alone gives the fleet its live half: ports, pids, holders. It cannot
give names, or say how to start something that is down. That is the roster —
a pluggable overlay behind one seam, =bebop-backline-roster-source=, default
nil. With no roster the fleet view still works, listing live listeners as
=port-NNNN=; with one, rows gain a name, a group, and two commands:

- =:start-cmd= — how a *session* runs the service inside its venue's
  backline window. May be nil: plenty of services have a build step or a jar
  workflow that belongs to the machine, not to a venue shell.
- =:machine-cmd= — how the *machine* runs or restores the service. This is
  what release runs to put a checked-out service back the way the machine
  had it.

Cardinality is set by the machine, not by bebop. On the iMac the fleet is a
dev server or two; on the Mac Pro it is the full microservice stack from
=~/dotfiles/roost/stack.sh=, whose =SERVICES= table the shipped adapter
(=bebop-backline-roster-stack-sh=) parses. Same abstraction either way.

** Per-project pinned ports

The exception to "no per-venue port allocation": most repos are port-agnostic,
so takeover-on-conflict is enough and a dev server can land on any free port.
A few repos pin their port for reasons
...
2, one file per machine) — a choice, not derivable state. So does the
=:sticky= flag that marks a set as hand-declared and exempt from the
empty-set sweep. Everything else remains derived. Assignment is designed
...
The sidecar generalizes: the old =bebop-mr-cache.json= became an
external-status cache holding =mr= and =pipeline= entries per session,
and the verb surface gained the pipeline write to match
...
Remote client for free — fetch once at home, render at the cafe. It
landed at =~/Code/Org/.bebop/HOST.status.json=, host-scoped like
=HOST.state.json= beside it, and there is no import from the old
un-scoped file: one machine's MR history is not another's.
...
The seam between bebop and whatever this machine uses to run its services.
=bebop-backline-roster= is the only reader: everything downstream sees a list
of plists and never learns where they came from. A roster source that is
missing, moved, or malformed costs one message and an empty roster — the
fleet view degrades to lsof discovery rather than erroring.
...
*** Roster adapter: stack.sh

The shipped implementation of the seam, and the reason the seam is a
function: =~/dotfiles/roost/stack.sh= already is this machine's registry —
its =SERVICES= table names every service, its type, its group, and its port,
and its verbs bring them up and down. Bebop reads that table rather than
keeping a second copy to drift.

What it declines to guess is as important as what it reads. Libraries and
=broken=-flagged rows are not services a session can hold. =:machine-cmd= is
always =stack.sh up NAME=, because that is exactly how the machine starts
one. =:start-cmd= is filled in only where the *type* decides it: =node= is
always =yarn dev=, =fe-npm= always =npm run dev=. =fe-yarn= is left nil
because stack.sh picks =yarn dev= or =yarn start= per service name and
sometimes builds icons first, and =maven= is left nil because its jar
workflow belongs to stack.sh. A wrong =:start-cmd= would have checkout run
the wrong thing in a real worktree; nil just says "start it yourself".

Set it up per machine — nothing turns it on by default, because the iMac does
not run the fleet the Pro does:

: (setq bebop-backline-roster-source #'bebop-backline-roster-stack-sh)
...
*** The fleet: derived holders

Who holds a service is computed on every read, in order of confidence: a
=backline--SLUG= pane in the listener's ancestry names its venue outright; a
cwd or command path under =bebop-venues-dir= names one less directly (a
server started from an agent's own pane, say); and a process neither of those
reaches belongs to the machine. There is no lease file to go stale.

Deriving on every read is only affordable if the reads are gathered. Asked
naively, sixty listeners cost sixty lsof calls, sixty ps walks, and a tmux
call per backline window per pid — seventeen seconds on this iMac, for a
table you want to glance at. So the four lookups the rule needs (listening
sockets, backline panes, the process tree, working directories) are each
taken once per fleet read and passed down as a context; the per-pid entry
points still work alone, for the one-off case.
...
=bebop-backline-status= stays the Lisp accessor =bebop-api= reads through:
per-venue work-shell facts, unchanged. Called interactively it has nothing
useful to return, so it shows =bebop-backline-fleet= instead — both layers in
one buffer, the machine's services over the venue shells that hold some of
them.

=bebop-backline-services= reports every listener, because a fleet view that
quietly drops rows is how a port conflict becomes a mystery. The *display*
is choosier: a real machine has dozens of machine-held ephemeral listeners
(Dropbox, LSP servers, simulators) that no session will ever check out, and
sixty rows of those hide the four that matter. So the table shows a row when
it is rostered, or held by a venue, or below the ephemeral floor — and says
...
Checkout is the fleet's verb: this session takes restart authority over one
service. Whoever held it loses that service and nothing else — the boot is an
interrupt inside their backline window, not a kill of the window, so the venue
keeps its work shell and its scrollback.

Release is the way back. It kills the process if this venue still holds the
service, then (by default) runs the roster's =:machine-cmd= so the machine
gets its own service back rather than a hole. That is what makes teardown
safe: a session can hold =payment= all afternoon and its exile leaves the
machine exactly as it found it.

Neither verb prompts. Both are called by agents through the emacs server and
by teardown with no human present, so the confirmation flow lives in
=bebop-backline-takeover= (the port-level verb a human drives) and these two
report instead: every boot names who lost the service.
...
pass, and it needs no approval: an empty enrichment-declared set holds
nothing, and enrichment re-declares it the moment a ticket matches its
epic again. Sticky sets — the ones a person declared by hand, which
nothing would re-declare — are skipped, so the unattended verb can
never delete a box someone built on purpose.
